# coding: utf-8

"""
    gnomock

    `gnomock` is an HTTP wrapper for [Gnomock](https://github.com/orlangure/gnomock) integration and end-to-end testing toolkit. It allows to use Gnomock outside of Go ecosystem. Not all Gnomock features exist in this wrapper, but official presets, as well as basic general configuration, are supported.   # noqa: E501

    The version of the OpenAPI document: 1.5.0
    Generated by: https://openapi-generator.tech
"""


from __future__ import absolute_import

import unittest
import datetime

import gnomock
from gnomock.models.influxdb_request import InfluxdbRequest  # noqa: E501
from gnomock.rest import ApiException

class TestInfluxdbRequest(unittest.TestCase):
    """InfluxdbRequest unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional):
        """Test InfluxdbRequest
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # model = gnomock.models.influxdb_request.InfluxdbRequest()  # noqa: E501
        if include_optional :
            return InfluxdbRequest(
                preset = gnomock.models.influxdb.influxdb(
                    version = 'latest', 
                    username = 'gnomock', 
                    password = 'gnomock-password', 
                    org = 'gnomock-org', 
                    bucket = 'gnomock-bucket', 
                    auth_token = 'gnomock-influxdb-token', ), 
                options = gnomock.models.options.options(
                    timeout = 60000000000, 
                    env = [
                        'ENV_VAR_NAME=some-value'
                        ], 
                    debug = True, 
                    container_name = 'gnomock', 
                    privileged = True, 
                    cmd = [
                        '0'
                        ], 
                    disable_cleanup = True, )
            )
        else :
            return InfluxdbRequest(
        )

    def testInfluxdbRequest(self):
        """Test InfluxdbRequest"""
        inst_req_only = self.make_instance(include_optional=False)
        inst_req_and_optional = self.make_instance(include_optional=True)


if __name__ == '__main__':
    unittest.main()
